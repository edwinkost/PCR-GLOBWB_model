...
def numpy2pcr(arg1, arg2, arg3):
    ...
    return lfr.from_numpy(np.transpose(np.float32(arg2)), config.partitionShape, arg3)

def pcr2numpy(arg1, arg2):
    # print("Note 'pcraster.numpy2pcr' function is PARTIALLY implemented on LUE yet.")
    return lfr.to_numpy(arg1)
...
def spatial(arg):
    if type(arg) == float:
        return lfr.create_array(config.array_shape, config.partitionShape, np.dtype(np.float32), fill_value = arg)
    else:
        return arg
