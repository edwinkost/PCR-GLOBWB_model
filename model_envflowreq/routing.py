...
class Routing(object):
    ...
    def update(self,landSurface,groundwater, dischargeEnvironmentalFlow, \
               volumeEnvironmentalFlow, dischargeBasedEnvironmentalFlow, \
               currTimeStep,meteo):
        ...
        self.downstreamDemand = dischargeEnvironmentalFlow
        ...
        self.readAvlChannelStorage = pcr.max(0.0, self.estimate_available_volume_for_abstraction(self.channelStorage, \
                                             dischargeEnvironmentalFlow, volumeEnvironmentalFlow, dischargeBasedEnvironmentalFlow))
    ...
    def get_channel_params(self):
        ...
        mannings_n       = self.manningsN
        channel_gradient = self.gradient
        channel_length   = self.channelLength

        if 'channeDepth' in vars(self).keys():

            channel_depth = self.channelDepth

        else:

            channel_depth = self.predefinedChannelDepth

        if 'channeWidth' in vars(self).keys():

            channel_width = self.channelWidth

        else:

            channel_width = self.predefinedChannelWidth

        return mannings_n, channel_gradient,channel_depth,\
               channel_width, channel_length


    def get_surface_water_area(self):

        # TODO: make sure these variables are set as part of the init-
        # ialization of the routing module
        # here, some preliminary checks and adaptations are made
        if not 'channelFraction' in vars(self).keys():

            mannings_n, channel_gradient,channel_depth,\
               channel_width, channel_length = self.get_channel_params()

            self.channelWidth = channel_width
            self.channelDepth = channel_depth
            cell_area         = self.cellArea

            self.channelFraction = pcr.max(0.0, pcr.min(1.0,\
                                   channel_width * channel_length / cell_area))

        if not 'channelStorageCapacity' in vars(self).keys():

            # get the channel storage capacity
            if self.usingFixedBankfullCapacity:
                self.channelStorageCapacity = self.predefinedBankfullCapacity
            else:
                self.channelStorageCapacity = self.estimateBankfullCapacity(self.channelWidth, \
        ...
        if not 'inundatedFraction' in vars(self).keys():

            # update flood fraction and flood depth
            self.inundatedFraction, self.floodDepth = self.returnInundationFractionAndFloodDepth(self.channelStorage)

        if not 'dynamic_fracwat' in vars(self).keys():
        ...
        surface_water_area = pcr.max(self.min_fracwat_for_water_height, \
                             self.dynamicFracWat) * self.cellArea

        return surface_water_area


    ####################################################################
    # update of the environmental flow requirements (RvB; 26-10-2022)   #
    # first follows the old function, renamed from                      #
    # estimate_available_volume_for_abstraction to                     #
    # estimate_available_volume_for_abstraction_old                    #
    ####################################################################

    def xestimate_discharge_for_environmental_flow(self, channelStorage):

        sys.exit('This function has been superseded by the EFR module!')
    ...
    def estimate_available_volume_for_abstraction_old(self, channelStorage, length_of_time_step = vos.secondsPerDay()):
    ...
    def estimate_available_volume_for_abstraction(self, \
                                                  channelStorage, \
                                                  dischargeEnvironmentalFlow, \
                                                  volumeEnvironmentalFlow, \
        ...
        # processing depend on whether the volume should be based on
        # the environmental flow requirements stated in terms of discharge
        # or, alternatively, on the volume
        if dischargeBasedEnvironmentalFlow:

            readAvlChannelStorage  = pcr.max(0.0,channelStorage)
            
            # get the discharge ratio based on the running-mean monthly discharge
            # note that the ratio is larger than 0 if the discharge is larger than
            # the environmental flow requirement
            ratio_efr_to_average_discharge = pcr.min(1.0, pcr.max(0 ,\
                                 vos.getValDivZero(self.avgDischargeShort, \
                                                   dischargeEnvironmentalFlow, vos.smallNumber) -1))
            
            # reset the ratio to unity where the reserved volume is zero,
            ratio_efr_to_average_discharge = pcr.ifthenelse(volumeEnvironmentalFlow == 0, \
                                                 pcr.scalar(1), ratio_efr_to_average_discharge)
                                                 
            # restrict the storage
            readAvlChannelStorage          = ratio_efr_to_average_discharge *\
                                             readAvlChannelStorage              
        else:
        ...
        # set a limit as a  (precentage) of water can be abstracted from
        # the channel in order to avoid avoid flip-flop
        maximum_percentage = 0.90
        readAvlChannelStorage = pcr.max(0, pcr.min(readAvlChannelStorage, \
                                        maximum_percentage * channelStorage))
    ...
    ####################################################################
    # end of the update of the environmental flow                       #
    ####################################################################
