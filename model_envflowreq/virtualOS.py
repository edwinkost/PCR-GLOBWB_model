...
def test_is_ncfile(filename):

    return os.path.splitext(filename)[1] in netcdf_suffixes

def test_nc_dimensions(filename, \
                       is_spatial, \
                       is_temporal, \
                       time_varname, \
                       lat_varname, \
    ...
    dim_info    = {'temporal':  [time_varname], \
                   'latitude':  [lat_varname, \
                                 lat_varname[:3]], \
                   'longitude': [lon_varname, \
                                 lon_varname[:3]]}
    ...
    try:
        ...
        # open and check the dataset structure
        f = nc.Dataset(filename)
        varnames = list(f.variables.keys())

        # check on the variables
        if isinstance(varname, NoneType):

            for dimnames in dim_info.values():

                for dimname in dimnames:

                    if dimname in varnames:

                        varnames.remove(dimname)

            varname = varnames[0]

        # check on the variable structure

        for dimname in dim_info['temporal']:

            if dimname in f.variables[varname].dimensions:
                is_temporal = True

                ix = list(f.variables[varname].dimensions).index(dimname)

                number_time_steps = f.variables[varname].shape[ix]

        for dimname in dim_info['latitude']:

            if dimname in f.variables[varname].dimensions:
                is_spatial = True

        for dimname in dim_info['longitude']:

            if dimname in f.variables[varname].dimensions:
                is_spatial = is_temporal and True

        # close the dataset
        f.close()

        # processed, set the error status
        error_on_process = False

    except:

        # netCDF file could not be processed, set the error status
        error_on_process = True

    # return the error status, as well as information
    # on the spatial and temporal status
    return is_spatial, is_temporal, number_time_steps, error_on_process


def test_file_structure(inputpath, filename, \
                       varname = None, \
                       time_varname = time_varname, \
                       lat_varname  = lat_varname, \
                       lon_var_name = lon_varname):
    ...
    if not isinstance(filename, NoneType) and not filename.lower() == 'none':
        ...
        if os.path.isfile(filename):
            ...
            is_ncfile =  test_is_ncfile(filename)

            if is_ncfile:

                is_spatial, is_temporal, \
                   number_time_steps, error_on_process = \
                            test_nc_dimensions(filename, \
                                               is_spatial, \
                                               is_temporal, \
                                               time_varname, \
                                               lat_varname, \
            ...
            if error_on_process:
                ...
                if co != '': is_spatial = True

        else:

            # assert that the information can be read as a float
            try:
            ...
            assert isinstance(entry, float), 'non-map input can only be a numerical value'

    return is_spatial, is_temporal, number_time_steps


class input_field(object):

    # NOTE: it is possible to pass the attributes via args or kwargs
    # but it is better to set the variables explicitly
    def __init__(self, \
                 name, \
                 variable_name, \
                 units, \
        ...
        self.allow_year_substitution = allow_year_substitution
        self.data_type               = data_type
        self.correction_constant     = correction_constant
        self.correction_factor       = correction_factor
        self.default_value           = default_value
        self.valid_range             = valid_range
        self.forced_non_spatial      = False

        # TODO: check the values and correct for undesirable values and
        # None values

        # TODO: set the variables in a general form and per type
        self.data_conv_func = pcr.scalar
        # at the moment only the Scalar type pcr.Scalar is admissible
        if self.data_type.name == 'Scalar':
            self.data_conv_func = pcr.scalar

        # check whether the input file, if any, is a netCDF file
        if isinstance(self.data, str):
            self.is_ncfile = test_is_ncfile(self.data)
        else:
            self.is_ncfile = False

        # set the filename as the data set
        # note that the inputpath then is no longer needed
        if isinstance(self.data, str):
        ...
        if isinstance(self.data, NoneType):

            self.value_field   = self.default_value
        ...
        if not self.forced_non_spatial and \
               not isinstance(self.value_field, NoneType):
            
            self.value_field = pcr.scalar(self.value_field)
    ...
    def set_value(self, value_field):
        
        self.value_field = value_field
        
        return self.__call__()

    def update(self, date, landmask, \
                     clone_filename, temppath, inputpath):

        # set the message str
        message_str = 'Variable %s' % self.name

        #set the value field
        if self.update_value:

            # set the variable field
            # NOTE / TODO: the test on the variable name can
            # be made elsewhere and uniquely, in that case repeated
            # testing can be avoided
            variable_name = self.variable_name
            if isinstance(variable_name, NoneType):
                variable_name = 'automatic'

            # read in the value as spatio-temporal field
            # or a single value

            if self.is_ncfile and self.is_temporal:

                # this assumes now that the file is a netCDF file and has
                # temporal input
                
                if self.allow_year_substitution:
                    useDoy = 'month'
                else:
                    useDoy = None

                self.value_field = netcdf2PCRobjClone( \
                                    ncFile            = self.data, \
                                    varName           = variable_name, \
                                    dateInput         = date, \
                                    useDoy            = useDoy, \
                                    cloneMapFileName  = clone_filename, \
                                    )

            elif self.is_ncfile:

                # this assumes now that the file is a netCDF file and
                # does not contain temporal input

                self.value_field = netcdf2PCRobjCloneWithoutTime( \
                                    ncFile            = self.data, \
                                    varName           = variable_name, \
                                    cloneMapFileName  = clone_filename, \
                                    )
                ...
                self.value_field = readPCRmapClone(self.data, \
                                                  clone_filename, \
                                                  temppath, inputpath)

            # correct the value, this is only done in the update
            # and before the default value is used to cover the area
            self.value_field = self.correction_constant + \
                              self.correction_factor   * \
                              self.value_field

            # TODO: add an exception for forced non-spatial data
            # and avoid to cast it as a PCRaster field clipped to the
            # land mask
            self.value_field = pcr.ifthen(landmask, \
                                  pcr.cover(self.data_conv_func(self.value_field), \
                                            self.default_value))

            # update the message string
            # set the sub-string with information
            if self.is_temporal:

                sub_str = 'using the nearest matching date'
                ...
                sub_str = 'reading in a connstant value or field'
            ...
            if isinstance(self.data, NoneType):
                ...
                sub_str = 'and the default value will be used'

            elif self.is_spatial or self.number_time_steps == 1:

                sub_str = str.join(' ', \
                                 ('for no specific temporal information',\
                                  'is available for this time step'))
            else:
        ...
        if self.is_temporal and self.number_time_steps > 1:
        ...
        # return value and message string
        return self.value_field, message_str
...
def scale_within_range(x, x_min, x_max):
    
    '''scale_within_range: function that scales the value x to the \
relative fraction between x_max and x_min of the form (x - x_min) / \
(x_max - x_min). The result is limited between 0 and 1.
'''

    # set the numerator and denominator and the valid range
    x_n = x - x_min
    x_d = x_max - x_min
    valid_range = x_min < x_max
    
    # this now works on PCRaster fields only
    # TODO: make this more flexible in terms of data type
    data_type = pcr._pcraster.Field

    
    if data_type == pcr._pcraster.Field: 

        if isinstance(valid_range, bool) and valid_range:
            
            x_f = pcr.scalar(x_n) / x_d
            
        elif isinstance(valid_range, bool) and not valid_range:
            
            x_f = pcr.scalar(0)
            
        elif isinstance(valid_range, pcr._pcraster.Field):
            
            x_f = pcr.ifthenelse(valid_range, pcr.scalar(x_n) / x_d, 0)
