...
class LandCover(object):
    ...
    def __init__(self,iniItems,nameOfSectionInIniFile,soil_and_topo_parameters,landmask,irrigationEfficiency,usingAllocSegments = False):
        ...
        if self.report:
            ...
            if 'netcdfAttributesOptions' in vars(iniItems).keys():
                logger.info("Passing specific netcdf attributes to the output files created")
                specificAttributeDictionary= iniItems.netcdfAttributesOptions
            else:
                specificAttributeDictionary= None
            #-initialize netcdfObj    
            self.netcdfObj = PCR2netCDF(iniItems, specificAttributeDictionary)
