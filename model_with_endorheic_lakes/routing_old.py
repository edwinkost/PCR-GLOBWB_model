...
class Routing(object):
    ...
    def __init__(self,iniItems,initialConditions,lddMap):
        ...
        if 'maximumLengthOfSubTimeStep' in iniItems.routingOptions.keys():
            maxiumLengthOfSubTimeStep = float(iniItems.routingOptions['maximumLengthOfSubTimeStep'])
            minimum_number_of_sub_time_step  = np.ceil(
                                               vos.secondsPerDay() / maximumLengthOfSubTimeStep )
    ...
    def kinematic_wave_update(self, landSurface, groundwater, currTimeStep, meteo): 
        ...
        for i_loop in range(number_of_loops):
            ...
            # RvB, October 2019: this section sets the discharge for the sub time step to
            # zero for lakes and reservoirs.
            self.subDischarge = pcr.ifthenelse(pcr.cover(self.WaterBodies.waterBodyIds, 0) == 0, \
                                self.subDischarge, pcr.scalar(0))
