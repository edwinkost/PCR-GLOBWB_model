...
class Reporting(object):
    ...
    def post_processing(self):
        ...
        #-RvB 7/02/2020: post-processing for the Global Nutrient Model (GNM)
        self.gnm_post_processing()
    ...
    def basic_post_processing(self):
        ...
        #-----------------------------------------------------------------------
        # NOTE (RvB, 28/03/2019): the following has been changed to include 
        # satAreaFrac: this is the area of the land surface over which the soil
        # is fully saturated as determined with the improved Arno scheme
        #
        self.satAreaFrac          = self._model.landSurface.satAreaFrac
        #-----------------------------------------------------------------------
    ...
    def additional_post_processing(self):
        ...
        #~ # consumptive water use and return flow from irrigation water demand (unit: m/day)  
        # Note that this variable does not exist!
        #~ self.irrReturnFlow          = self._model.landSurface.irrReturnFlow
        ...
        if self._model.routing.floodPlain:
            ...
            self.floodArea = pcr.ifthen(self._model.routing.landmask, \
                        pcr.ifthenelse(pcr.cover(self._model.routing.WaterBodies.waterBodyIds,0) == 0,\
                        self._model.routing.dynamicFracWat, 0)) * self._model.routing.cellArea
        else:
            self.floodDepth  = pcr.ifthen(self._model.routing.landmask, \
    ...
    def gnm_post_processing(self):

        ########################################################################
        # Global Nutrient Model - GNM
        ########################################################################
        # RvB 07/02/2020: post-processing of variables to be used as input in
        # the Global Nutrient Model
        # (GNM: Beusen 2015, 10.5194/gmd-8-4045-2015)
        #
        ...
        # get the variables of the water bodies
        # these are read and covered by zeros to avoid missing cells
        # (cover operation is obsolete now the maps are filled completely)
        waterBodyIds      = pcr.cover(self._model.routing.WaterBodies.waterBodyIds,0)
        waterBodyTyp      = pcr.cover(self._model.routing.WaterBodies.waterBodyTyp,0)
        waterBodyLoc      = pcr.cover(self._model.routing.WaterBodies.waterBodyLoc,0)
        waterBodyOut      = pcr.cover(self._model.routing.WaterBodies.waterBodyOut,0)
        endorheicLakes    = pcr.cover(self._model.routing.WaterBodies.endorheicLakes,0)
        waterBodyCellArea = pcr.cover(self._model.routing.WaterBodies.waterBodyCellArea,0)

        # raise error if dynamic floodplain is not enabled
        if not self._model.routing.floodPlain:
            logger.warning('This does not work! GNM post-processing requires that the dynamic floodplain is enabled.\n Only zeros are returned!')

            # state variables: information is reported for the endorheic water bodies,
            # those areas where water stagnates at the end of the drainage network,
            # and the active areas where water actively flows across the surface
            #
            # area [m2]
            self.active_water_area = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))
            self.endorheic_water_area = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))
            self.total_active_water_area = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))
            self.total_endorheic_water_area = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))

            # total water storage m3]
            self.active_water_volume = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))
            self.endorheic_water_volume = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))
            self.total_active_water_volume = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))
            self.total_endorheic_water_volume = pcr.ifthen(self._model.routing.landmask, \
                                                        pcr.scalar(0))
            ...
            #
            # area [m2]
            # for the active area, this is the area outside the endorheic lakes and may
            # be flooded (in the case of channels) or be a lake or reservoir, note that this is 
            # computed per cell
            self.active_water_area = pcr.ifthen(self._model.routing.landmask, \
                            pcr.ifthenelse(waterBodyIds == 0,\
                                pcr.max(self._model.routing.channelFraction * \
                                    self._model.routing.cellArea, self.floodArea), \
                                pcr.ifthenelse(pcr.pcrnot(endorheicLakes), \
                                    waterBodyCellArea, 0)))
            self.endorheic_water_area = pcr.ifthen(self._model.routing.landmask, \
                pcr.ifthenelse(endorheicLakes, \
                                    waterBodyCellArea, 0))
            # total area per body [m2]
            self.total_active_water_area = pcr.ifthenelse(waterBodyIds != 0, \
                    pcr.areatotal(self.active_water_area, waterBodyIds), \
                        self.active_water_area)
            self.total_endorheic_water_area = pcr.ifthenelse(waterBodyIds != 0, \
                    pcr.areatotal(self.endorheic_water_area, waterBodyIds), \
                        self.endorheic_water_area)

            # total water storage m3]
            self.total_active_water_volume = pcr.ifthen(self._model.routing.landmask, \
                            pcr.max(0, pcr.ifthenelse(waterBodyIds == 0,\
                                self.channelStorage, \
                            pcr.ifthenelse(pcr.pcrnot(endorheicLakes), \
                                pcr.cover(self.waterBodyStorage, 0), 0))))
            self.total_endorheic_water_volume = pcr.ifthen(self._model.routing.landmask, \
                            pcr.max(0, pcr.ifthenelse(endorheicLakes, \
                                pcr.cover(self.waterBodyStorage, 0), 0)))
            # correction to local volume on the basis of the ratio
            correction_factor = waterBodyCellArea / \
                                pcr.ifthenelse(waterBodyIds == 0,\
                                        pcr.scalar(1), \
                                    pcr.ifthenelse(endorheicLakes, \
                                        self.total_endorheic_water_area, \
                                        self.total_active_water_area))
            correction_factor = pcr.min(1.0, pcr.max(0.0, correction_factor))
            # and get the local values
            self.active_water_volume = correction_factor *\
                                       self.total_active_water_volume
            self.endorheic_water_volume = correction_factor *\
                                       self.total_endorheic_water_volume
            
        # and finally, set the outlets; this is independent of the routing 
        # floodplain option selected
        self.waterBodyOutType = pcr.scalar( \
                        pcr.ifthen(self._model.routing.landmask, \
                            pcr.ifthenelse(waterBodyLoc != 0,
                                pcr.scalar(waterBodyTyp), 0) * \
                                pcr.ifthenelse(endorheicLakes, \
                                    pcr.scalar(-1), 1)))
