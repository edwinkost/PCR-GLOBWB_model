...
def boolean(arg):
    ...
    if arg == 1.0:
        ...
        return lfr.create_array(array_shape, partition_shape, np.dtype(np.bool_), fill_value = 1)
    elif arg == 0.0:
        return lfr.create_array(array_shape, partition_shape, np.dtype(np.bool_), fill_value = 0)
    else:
        print(arg)
        raise NotImplementedError
