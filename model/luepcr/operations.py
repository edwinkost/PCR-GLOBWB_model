...
def boolean(arg):
    ...
    if arg == 1.0:
        ...
        return lfr.valid(lfr.create_array(array_shape, partition_shape, np.dtype(np.float32), fill_value = 0.0)) * lfr.create_array(array_shape, partition_shape, np.dtype(np.uint8), fill_value = 0)
    else:
        print(arg)
        raise NotImplementedError
