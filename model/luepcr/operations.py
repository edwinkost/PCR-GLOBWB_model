...
def cover(arg1, arg2, check_with_aguila = False):
    if check_with_aguila:
        ...
        #~ aguila(valid_check)
        print("after cover")
        check_arg1 = lfr.create_array(array_shape, partition_shape, np.dtype(np.float32), fill_value = 0.5)
        check_arg2 = lfr.create_array(array_shape, partition_shape, np.dtype(np.float32), fill_value = float(arg2))
        print(arg2)
