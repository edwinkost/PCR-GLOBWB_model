...
def mergeNetCDF(inputTuple):
    ...
    # - create dimensions and variables for longitudes and latitudes 
    ...
    if len(calendar_used) == 0:
        varStructure = ('lat','lon')  
    else:
        varStructure = ('time','lat','lon')  
    ...
    for time in uniqueTimes[:]:
        ...
        for ncFile in list(netCDFInput.values()):
            ...
            # latitudes and longitudes for selecting rows and cols, rounded until 4 decimals (TODO: You may want to improve this to 5 decimals, particularly for the resolution < 1 km (0.0083333333 arcdgree)
            lat_for_selection = np.around(latitudes,  decimals=4) 
            lon_for_selection = np.around(longitudes, decimals=4) # TODO: Improve this. We need this one for selecting rows and columns. 
            ...
            row0 = np.where(lat_for_selection == min(latMax,latMaxNcFile))[0][0]
            row1 = np.where(lat_for_selection == max(latMin,latMinNcFile))[0][0]+1
            col0 = np.where(lon_for_selection == max(lonMin,lonMinNcFile))[0][0]
            col1 = np.where(lon_for_selection == min(lonMax,lonMaxNcFile))[0][0]+1
...
deltaLat        = np.double(5.0/60.0)
deltaLon        = np.double(5.0/60.0)
latMin          = np.double( -90.0 + deltaLat / 2)
...
if sys.argv[11] == "defined":
    ...
    lonMinString = sys.argv[12]
    lonMaxString = sys.argv[13]
    latMinString = sys.argv[14]
    latMaxString = sys.argv[15]
    deltaString  = sys.argv[16]
